...
export default async function CaregiverResourcesPage() {
  ...
  const TabButton = ({ label, icon: Icon, active }: any) => (
    <button className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${active ? "bg-white text-teal-600 shadow-md ring-1 ring-slate-100" : "text-slate-400 hover:text-slate-600"}`}>
  ...
  return (
    <div className="space-y-12 selection:bg-teal-100 selection:text-teal-900 pb-20">
      ...
      <div className="space-y-2">
         <h1 className="text-4xl font-bold font-outfit tracking-tight text-slate-900 italic italic">Guides & <span className="text-teal-600">Support</span></h1>
         <p className="text-sm text-slate-500 font-medium italic italic">Educational materials for both clinical coordination and your own wellbeing.</p>
      </div>
      ...
      <div className="space-y-16">
         ...
         <div className="space-y-8">
            <div className="flex items-center justify-between">
               <h3 className="text-2xl font-bold font-outfit text-slate-900 flex items-center gap-3 italic italic border-b border-teal-50 pb-2">
               ...
               <div className="flex items-center gap-3 bg-teal-50 px-4 py-2 rounded-2xl border border-teal-100/50">
                  ...
                  <p className="text-[10px] font-black text-teal-800 uppercase tracking-widest leading-none">Shared with you by the care team</p>
               </div>
            </div>
                  ...
                  <GlassCard key={guide.title} className="!p-8 group cursor-pointer hover:border-teal-100 transition-all shadow-md bg-white">
                     <div className="flex flex-col h-full justify-between gap-8">
                        <div className="space-y-4">
                           <div className="flex items-center justify-between">
                              <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-slate-400 text-lg group-hover:bg-teal-50 group-hover:text-teal-600 transition-colors shadow-sm">
                           ...
                           <h4 className="text-xl font-bold font-outfit text-slate-900 leading-tight italic italic underline decoration-slate-100 group-hover:decoration-teal-200 transition-all underline-offset-4">{guide.title}</h4>
                           <p className="text-xs text-slate-500 font-medium italic italic leading-relaxed">Read the same materials {patientName.split(' ')[0]} reads to stay informed during clinical visits.</p>
                        </div>
                        <div className="flex items-center justify-between pt-6 border-t border-slate-50 group-hover:border-teal-100/50 transition-colors">
                           <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{guide.time} • {guide.type}</span>
                           <Button variant="ghost" className="h-9 px-6 text-[10px] font-black uppercase tracking-widest text-teal-600 hover:bg-teal-50 border border-teal-50 transition-all">Read Now</Button>
                        </div>
                     </div>
                  </GlassCard>
               ))}
         ...
         <div className="space-y-8 pt-8">
            <h3 className="text-2xl font-bold font-outfit text-slate-900 flex items-center gap-3 border-b border-rose-50 pb-2 italic italic">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="md:col-span-2 space-y-6">
                  {[
                     ...
                     <GlassCard key={cguide.title} className="!p-6 flex items-center justify-between group hover:border-teal-100 transition-all bg-white shadow-sm">
                         <div className="space-y-2">
                            <h4 className="text-base font-bold text-slate-900 italic italic">{cguide.title}</h4>
                            <p className="text-xs text-slate-500 font-medium italic italic max-w-md">{cguide.desc}</p>
                         </div>
               ...
               <div className="p-8 bg-slate-50 rounded-[40px] border border-slate-100 space-y-8">
                  <h4 className="text-[11px] font-black uppercase text-slate-400 tracking-[0.2em] italic italic">External Support</h4>
         ...
         <GlassCard className="bg-slate-950 text-white border-0 shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col items-center justify-center text-center p-4">
               <p className="text-[10px] font-black uppercase tracking-[0.3em] text-teal-500 mb-6 font-serif">Self-care reminder</p>
               <h3 className="text-3xl font-bold font-outfit tracking-tight leading-snug italic italic max-w-2xl px-4 text-teal-50 ring-1 ring-white/5 py-4 rounded-[40px]">
                  "{randomQuote}"
               </h3>
               <div className="mt-8 flex items-center gap-4 text-slate-500">
                  <div className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-pulse" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 italic italic">Rotating Support Engine v2.4</p>
               </div>
            </div>
         </GlassCard>
      </div>
