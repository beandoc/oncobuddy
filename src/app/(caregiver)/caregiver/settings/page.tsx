...
export default async function CaregiverSettingsPage() {
  ...
  const NavItem = ({ label, icon: Icon, active }: any) => (
    <button className={`w-full flex items-center justify-between p-5 rounded-2xl transition-all border ${active ? 'bg-teal-600 border-teal-500 text-white shadow-sm shadow-teal-100' : 'bg-white border-slate-100 text-slate-500 hover:bg-slate-50'}`}>
       <div className="flex items-center gap-4">
          <Icon className={`w-5 h-5 ${active ? 'text-white' : 'text-slate-400'}`} />
          <span className="text-sm font-bold uppercase tracking-wider leading-none">{label}</span>
       </div>
  ...
  return (
    <div className="space-y-12 selection:bg-teal-100 selection:text-teal-900 pb-20">
      ...
      <div className="space-y-2">
         <h1 className="text-4xl font-bold font-outfit tracking-tight text-slate-900">My <span className="text-teal-600">Preferences</span></h1>
         <p className="text-sm text-slate-500 font-medium">Manage your profile information and active patient connections.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
         ...
         <div className="space-y-3">
            ...
            <div className="pt-8 space-y-4">
               <button className="w-full h-14 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-center gap-3 text-rose-600 text-xs font-bold uppercase tracking-wider hover:bg-rose-100 transition-all shadow-sm active:scale-95">
         ...
         <div className="lg:col-span-2 space-y-12 h-full">
            ...
            <div className="space-y-8 h-full pb-8 border-b border-slate-100/50">
               <div className="flex items-center gap-8 relative group">
                  <div className="w-32 h-32 rounded-full border-4 border-white shadow-sm bg-slate-100 flex items-center justify-center font-bold text-slate-300 text-5xl relative overflow-hidden group">
                  ...
                  <div className="space-y-4 flex-1">
                     <div className="space-y-1">
                        <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider leading-none">Your Display Name</p>
                        <p className="text-2xl font-bold font-outfit text-slate-900 border-b-2 border-slate-50 pb-2 flex items-center justify-between">
                           {session.user.name}
                           <button className="text-[10px] font-bold uppercase text-teal-600 tracking-wider hover:text-slate-950 transition-colors">Edit</button>
                        </p>
                     </div>
                     <div className="flex items-center gap-6">
                        <div className="space-y-1">
                           <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider leading-none">Contact Phone</p>
                           <p className="text-sm font-bold text-slate-700 mt-1 italic underline decoration-slate-100 group-hover:decoration-teal-200 transition-all underline-offset-4">+91 98888 88888</p>
                        </div>
                        <div className="space-y-1">
                           <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider leading-none">Email Address</p>
                           <p className="text-sm font-bold text-slate-700 mt-1 italic underline decoration-slate-100 group-hover:decoration-teal-200 transition-all underline-offset-4 opacity-70 cursor-not-allowed">{session.user.email}</p>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <div className="space-y-8 pt-4">
               <div className="flex items-center justify-between">
                   <h3 className="text-2xl font-bold font-outfit text-slate-900 italic leading-none underline decoration-slate-100 underline-offset-8">Patient Connections</h3>
                   <div className="p-1 px-4 bg-teal-50 rounded-full border border-teal-100">
                      <p className="text-[10px] font-bold text-teal-600 uppercase tracking-wider animate-pulse leading-none">Verification Synchronized</p>
                   </div>
               </div>
               ...
               <div className="space-y-4">
                  {caregiver.patients.map((connection: any) => (
                     <GlassCard key={connection.id} className="!p-8 group hover:border-teal-100 transition-all shadow-md bg-white">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
                           <div className="flex items-center gap-6">
                              <div className="w-16 h-16 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center font-bold text-slate-400 shadow-sm transition-all  group-hover:bg-white group-hover:border-teal-100">
                              ...
                              <div className="space-y-2">
                                 ...
                                 <div className="flex items-center gap-3">
                                    <span className="text-[10px] font-bold uppercase text-teal-600 tracking-wider font-serif leading-none">{connection.relationship}</span>
                                    <div className="w-1 h-1 rounded-full bg-slate-200" />
                                    <span className={`text-[10px] font-bold uppercase tracking-wider leading-none ${connection.accessLevel === 'VIEW_AND_LOG' ? 'text-teal-600' : 'text-slate-400'}`}>
                                       {connection.accessLevel === 'VIEW_AND_LOG' ? 'Edit Access' : 'View Only'}
                                    </span>
                                 </div>
                                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1">Access granted: {new Date(connection.createdAt).toLocaleDateString()}</p>
                              </div>
                           </div>
                  ...
                  {caregiver.patients.length === 0 && (
                     <div className="p-16 border-2 border-dashed border-slate-100 rounded-[48px] text-center opacity-30 select-none">
                        ...
                        <p className="text-sm font-bold text-slate-400 font-outfit uppercase tracking-[0.2em]">No active patient connections found</p>
                     </div>
                  )}
               </div>
            </div>
            ...
            <GlassCard className="!p-10 border-slate-100 shadow-sm bg-slate-50/50 space-y-6 relative overflow-hidden group">
               ...
               <div className="pt-8 space-y-3 relative z-10">
                  <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider leading-none">Account Deletion</p>
                  <button className="text-[10px] font-bold uppercase text-rose-500 tracking-wider hover:underline decoration-rose-300 underline-offset-4 decoration-2 decoration-solid decoration-skip-ink-none">Request account anonymization and removal</button>
               </div>
            </GlassCard>
         </div>
      </div>
