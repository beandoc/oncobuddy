    ...
    <div className="flex h-[calc(100vh-180px)] gap-6 animate-in fade-in duration-700">
      ...
      <div className="w-64 space-y-8 flex flex-col h-full bg-slate-50/30 p-4 rounded-3xl border border-slate-100/50">
         <div className="space-y-1 px-2">
            <h1 className="text-2xl font-bold font-outfit text-slate-900 border-b-2 border-indigo-600 pb-2 italic">Clinical <span className="text-indigo-600">Comms</span></h1>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest pt-2">Internal Secure Triage</p>
         </div>

         <div className="flex-1 space-y-2">
            <button className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-indigo-600 text-white shadow-xl shadow-indigo-100 transition-all hover:scale-[1.02]">
               <div className="flex items-center gap-3">
                  <Inbox className="w-4 h-4" />
                  <span className="text-[11px] font-black uppercase tracking-widest leading-none">Inbox</span>
               </div>
               <span className="text-[9px] font-black bg-white/20 px-2 py-0.5 rounded-full">12</span>
            </button>
            <button className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-white border border-slate-100 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-black uppercase tracking-widest leading-none">Urgent Breaks</span>
               </div>
               <span className="text-[9px] font-black bg-rose-100 text-rose-600 px-2 py-0.5 rounded-full">3</span>
            </button>
            <button className="w-full flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-slate-100 text-slate-500 hover:bg-slate-50 transition-all">
               ...
               <span className="text-[11px] font-black uppercase tracking-widest leading-none">Nurse Teams</span>
            </button>
            <button className="w-full flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-slate-100 text-slate-500 hover:bg-slate-50 transition-all">
               <Star className="w-4 h-4" />
               <span className="text-[11px] font-black uppercase tracking-widest leading-none">Starred</span>
            </button>
            <button className="w-full flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-slate-100 text-slate-500 hover:bg-slate-50 transition-all">
               <Trash2 className="w-4 h-4" />
               <span className="text-[11px] font-black uppercase tracking-widest leading-none">Archive</span>
            </button>
         </div>

         <div className="pt-8 space-y-4">
            <GlassCard className="!p-4 bg-indigo-50/50 border-indigo-100 shadow-none text-center">
               <p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mb-3 italic">Shift Continuity</p>
               <Button className="w-full h-10 bg-indigo-600 text-white font-black text-[9px] uppercase tracking-widest shadow-xl shadow-indigo-100">Hand-off Report</Button>
            </GlassCard>
         </div>
      </div>
      ...
      <div className="w-80 border border-slate-100 rounded-3xl bg-white flex flex-col h-full shadow-inner shadow-slate-50">
         <div className="p-6 border-b border-slate-50 space-y-4">
            ...
            <div className="flex items-center justify-between">
               <span className="text-[10px] font-black uppercase text-slate-300 tracking-widest italic">Sorted: Recent</span>
            ...
            {[
               ...
               <div key={idx} className={`p-4 mx-3 rounded-2xl transition-all cursor-pointer group flex items-start gap-3 mt-1 ${msg.unread ? 'bg-indigo-50/30' : 'hover:bg-slate-50'}`}>
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                     <div className="flex items-center justify-between">
                        <p className="text-[11px] font-black text-slate-900 leading-none">{msg.name}</p>
                        <span className="text-[8px] font-bold text-slate-400 whitespace-nowrap">{msg.time}</span>
                     </div>
                     <p className="text-[10px] font-bold text-indigo-600 truncate italic">Patient: {msg.p}</p>
      ...
      <div className="flex-1 flex flex-col h-full bg-white rounded-3xl border border-slate-100 shadow-2xl relative overflow-hidden group">
         {/* Conversation Header */}
         <div className="p-6 border-b border-slate-100/50 flex items-center justify-between">
            <div className="flex items-center gap-4">
               <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-bold shadow-xl shadow-indigo-100">M</div>
               <div>
                  <h3 className="text-base font-bold font-outfit text-slate-900 italic italic leading-none">Nurse Maya J.</h3>
                  <div className="flex items-center gap-2 mt-1.5 leading-none">
                     <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                     <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest italic">On-Shift Active</span>
                  </div>
               </div>
            </div>
            <div className="flex items-center gap-3">
         ...
         <div className="flex-1 overflow-y-auto p-10 space-y-10 scroll-smooth no-scrollbar">
            ...
            <div className="flex justify-center flex-col items-center gap-3">
               ...
               <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.3em] font-serif">Today, October 12th</p>
            </div>
            ...
            <div className="flex items-start gap-4 max-w-[80%]">
               ...
               <div className="space-y-2">
                  <div className="p-5 rounded-[28px] rounded-tl-none bg-indigo-50 text-[13px] text-slate-700 leading-relaxed font-outfit italic shadow-sm hover:translate-x-1 transition-all">
                  ...
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest pl-2">Maya J. • 12:45 PM</p>
               </div>
            </div>
            ...
            <div className="flex items-start gap-4 max-w-[80%] ml-auto flex-row-reverse">
               ...
               <div className="space-y-2 text-right">
                  <div className="p-5 rounded-[28px] rounded-tr-none bg-slate-900 text-[13px] text-white leading-relaxed font-outfit italic shadow-xl">
                  ...
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest pr-2">You • 12:50 PM</p>
               </div>
            </div>
            ...
            <div className="mx-auto w-full max-w-sm">
               <GlassCard className="!p-4 bg-amber-50/50 border-amber-200/50 shadow-none flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                     <p className="text-[10px] font-black text-amber-800 uppercase tracking-[0.2em] italic">Breach Awareness</p>
                     <p className="text-[11px] text-amber-700 font-medium italic italic">John Doe has a pending lab breach since 9:14am.</p>
                  </div>
               </GlassCard>
            </div>
         </div>
         ...
         <div className="p-6 pt-0 space-y-6">
            <div className="flex items-center gap-3">
               <Button variant="outline" className="h-9 px-6 rounded-full border-indigo-100 text-indigo-600 text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 shadow-sm transition-all focus:ring-4 ring-indigo-50">Hydration Adj</Button>
               <Button variant="outline" className="h-9 px-6 rounded-full border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 shadow-sm transition-all">Sign Labs</Button>
               <Button variant="outline" className="h-9 px-6 rounded-full border-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 shadow-sm transition-all">Notify Floor</Button>
            </div>
            
            <div className="p-4 bg-slate-50/50 rounded-[32px] border border-slate-100/50 flex items-center gap-4 focus-within:bg-white focus-within:border-indigo-200 transition-all shadow-inner group">
               ...
               <input type="text" placeholder="Consult with care team..." className="flex-1 bg-transparent border-none focus:ring-0 text-[13px] font-medium font-outfit italic placeholder:text-slate-300 outline-none" />
               <Button className="h-12 w-12 p-0 bg-indigo-600 hover:bg-slate-950 text-white rounded-[20px] shadow-xl shadow-indigo-100 transition-all active:scale-90 flex items-center justify-center">
