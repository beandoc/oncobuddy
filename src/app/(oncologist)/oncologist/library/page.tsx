    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
         <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium italic opacity-80 pt-2">Curated clinical education and rehab pathways for patient assignment.</p>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
         {[
            ...
            <GlassCard key={i} className="group hover:bg-white border-white/50 hover:border-indigo-100 hover:shadow-sm transition-all cursor-pointer !p-8 rounded-xl">
               ...
               <p className="text-[10px] font-bold uppercase text-indigo-400 tracking-wider mb-2 font-serif">{asset.category}</p>
               <h3 className="text-xl font-bold font-outfit text-slate-900 mb-4 group-hover:text-indigo-600 transition-colors">{asset.title}</h3>
               <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{asset.duration}</span>
                  <div className="flex items-center gap-2 text-[10px] font-bold text-indigo-600 uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">Assign <ChevronRight className="w-4 h-4" /></div>
               </div>
            </GlassCard>
         ))}
      </div>
