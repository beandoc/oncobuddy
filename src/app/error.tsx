...
export default function GlobalError({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 selection:bg-rose-100 selection:text-rose-900">
      <GlassCard className="max-w-2xl w-full !p-12 space-y-10 border-rose-100 bg-white shadow-sm shadow-rose-100/50 relative overflow-hidden group">
         ...
         <div className="flex flex-col items-center text-center space-y-6 relative z-10">
            <div className="w-20 h-20 rounded-3xl bg-rose-50 border-4 border-white shadow-sm flex items-center justify-center text-rose-600 animate-pulse">
               <AlertCircle className="w-10 h-10" />
            </div>
            
            <div className="space-y-3">
               <h1 className="text-3xl font-bold font-outfit text-slate-900 tracking-tight underline decoration-rose-500/20 underline-offset-8">
         ...
         <div className="bg-rose-50/50 border border-rose-100 rounded-3xl p-8 space-y-4">
            ...
            <p className="text-[13px] text-rose-800 leading-relaxed font-medium underline decoration-rose-200 decoration-1 underline-offset-4">
            ...
            <div className="flex items-center gap-4 pt-2">
               <Button variant="outline" className="h-11 px-6 gap-3 text-[10px] font-bold uppercase tracking-wider border-rose-200 bg-white text-rose-600 shadow-sm transition-all hover:bg-rose-50">
         ...
         <div className="flex flex-col items-center gap-4 pt-4">
            <Button 
               onClick={() => reset()}
               className="h-14 w-full bg-slate-900 border-0 text-white font-bold text-xs uppercase tracking-wider shadow-sm hover:scale-105 active:scale-95 transition-all group"
            >
               <RefreshCcw className="w-4 h-4 group-hover:rotate-180 transition-transform duration-700" /> Reload Clinical Workspace
            </Button>
            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-wider font-serif">Error Code: {error.digest || 'RUNTIME_FAULT'}</p>
         </div>
      </GlassCard>
    </div>
  );
