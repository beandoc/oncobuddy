...
export default async function PatientMoreHub() {
  ...
  const MenuLink = ({ href, icon: Icon, label, sublabel, destructive = false }: any) => (
    <Link 
      href={href} 
      className={`p-6 rounded-xl border-2 flex items-center justify-between group transition-all active:scale-[0.98] ${destructive ? 'border-rose-100 bg-rose-50/20' : 'border-slate-100 bg-white shadow-sm hover:border-indigo-100'}`}
    >
       <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
             <p className={`text-base font-bold uppercase leading-none ${destructive ? 'text-rose-700' : 'text-slate-900'}`}>{label}</p>
             <p className={`text-[10px] font-bold uppercase tracking-wider ${destructive ? 'text-rose-400' : 'text-slate-400'}`}>{sublabel}</p>
          </div>
       </div>
  ...
  return (
    <div className="max-w-xl mx-auto space-y-10 animate-in fade-in duration-700 pb-32 md:hidden">
      ...
      <div className="flex flex-col items-center text-center space-y-4 pt-4">
         <div className="w-20 h-20 rounded-xl bg-slate-950 flex items-center justify-center shadow-sm shadow-indigo-100">
            <ShieldCheck className="w-10 h-10 text-white" />
         </div>
         <div className="space-y-1">
            <h1 className="text-3xl font-bold font-outfit text-slate-900 tracking-tighter">Oncobuddy</h1>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Patient Clinical Environment v2.4.0</p>
         </div>
      </div>

      {/* Identity Summary Card */}
      <GlassCard className="!p-8 border-slate-100 shadow-sm rounded-xl flex items-center gap-6 bg-white/70">
         <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center font-bold text-white text-2xl shadow-sm shadow-indigo-100">
            {session.user.name?.charAt(0)}
         </div>
         <div className="flex-1">
            <p className="text-xl font-bold text-slate-900 leading-none">{session.user.name}</p>
            <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider mt-2 bg-indigo-50 px-2 py-0.5 rounded-md inline-block">Active Treatment Cycle</p>
         </div>
      </GlassCard>

      {/* Primary Navigation Cluster */}
      <div className="space-y-4">
         <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider ml-6">Navigation Vectors</h3>
      ...
      <div className="space-y-4 pt-4">
         <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider ml-6">Support & Safety</h3>
         <div className="grid grid-cols-1 gap-3">
