  ...
  const MsgBubble = ({ sender, text, time, self }: any) => (
    <div className={`max-w-[85%] md:max-w-[70%] space-y-2 ${self ? 'self-end' : 'self-start'}`}>
       <div className={`flex items-center gap-3 px-2 ${self ? 'flex-row-reverse' : 'flex-row'}`}>
          <p className="text-[10px] font-black uppercase text-slate-950 tracking-[0.2em]">{sender}</p>
          <div className="w-1 h-1 rounded-full bg-slate-200" />
          <p className="text-[10px] font-bold text-slate-400 uppercase">{time}</p>
       </div>
       <div className={`p-6 rounded-[32px] text-base font-black leading-relaxed italic italic shadow-xl ${self ? 'bg-indigo-600 text-white rounded-br-none shadow-indigo-100' : 'bg-white border-2 border-slate-50 text-slate-900 rounded-bl-none shadow-slate-100'}`}>
          {text}
       </div>
    </div>
  );
    ...
    <div className="h-[calc(100vh-16rem)] flex flex-col lg:flex-row gap-10 selection:bg-indigo-100 selection:text-indigo-900 animate-in fade-in duration-700">
      ...
      <div className="w-full lg:w-96 flex flex-col gap-8 h-full">
         <div className="flex items-center justify-between border-b-4 border-slate-100 pb-6 px-2">
            <h1 className="text-3xl font-black font-outfit text-slate-950 italic">Broadcast</h1>
            <button className="w-12 h-12 bg-slate-950 rounded-2xl flex items-center justify-center text-white shadow-2xl hover:bg-indigo-600 transition-all active:scale-95"><PlusCircle className="w-6 h-6" /></button>
         </div>
         ...
         <div className="relative group px-1">
            ...
            <input 
               ...
               className="w-full pl-14 pr-6 py-4 bg-slate-100/50 border-2 border-transparent focus:border-indigo-100 focus:bg-white rounded-[24px] text-sm font-black italic tracking-tight transition-all placeholder:text-slate-400 focus:ring-0" 
            />
         </div>

         <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 pb-24 lg:pb-0 px-1">
            {mockThreads.map(t => (
               <div key={t.id} className={`p-6 rounded-[36px] border-2 transition-all cursor-pointer group relative active:scale-[0.98] ${t.unread ? 'bg-indigo-50/20 border-indigo-100 shadow-xl shadow-indigo-50/30' : 'bg-white border-slate-50 hover:border-slate-100'}`}>
                  <div className="flex justify-between items-start mb-3">
                     <p className="text-base font-black text-slate-950 leading-none group-hover:text-indigo-600 transition-colors italic">{t.sender}</p>
                     <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.time}</span>
                  </div>
                  <p className="text-[10px] font-black uppercase text-indigo-600 tracking-[0.3em] mb-3 italic">{t.role}</p>
                  <p className="text-sm text-slate-600 line-clamp-1 font-bold italic italic">{t.lastMsg}</p>
      ...
      <GlassCard className="flex-1 !p-0 border-2 border-slate-100 shadow-[0_45px_90px_rgba(0,0,0,0.06)] relative flex flex-col h-full bg-white rounded-[48px] overflow-hidden">
         ...
         <div className="p-8 border-b-2 border-slate-50 flex items-center justify-between bg-white/95 backdrop-blur-xl relative z-20">
            <div className="flex items-center gap-6">
               <div className="w-16 h-16 rounded-[22px] bg-slate-950 flex items-center justify-center font-black text-white text-2xl shadow-2xl transition-transform hover:rotate-3">NM</div>
               <div className="space-y-2">
                  <p className="text-xl font-black text-slate-950 leading-none italic italic">Nurse Maya</p>
                  <div className="flex items-center gap-3">
                     <div className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]" />
                     <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none">Primary Navigator • Responsive</p>
                  </div>
               </div>
            </div>
         ...
         <div className="flex-1 overflow-y-auto p-10 flex flex-col gap-12 no-scrollbar relative bg-slate-50/30">
            ...
            <div className="self-center py-2.5 px-8 bg-white border-2 border-slate-100 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-6 shadow-sm">Monday, 10 March</div>
            ...
            <div className="py-2 px-6 self-end flex items-center gap-3">
               <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest italic italic">Broadcast Delivered • Read 11:51 AM</span>
            ...
            <div className="space-y-8">
               ...
               <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2">
                  {['General Inquiry', 'Symptom Triage', 'Medication Help', 'Visit Change', 'Emergency Spike'].map(type => (
                     <button key={type} className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest border-2 transition-all whitespace-nowrap active:scale-95 ${type === 'Emergency Spike' ? 'bg-rose-50 border-rose-100 text-rose-600 hover:bg-rose-600 hover:text-white' : 'bg-slate-50 border-slate-50 text-slate-600 hover:border-indigo-100 hover:text-indigo-600'}`}>
               ...
               <div className="relative group">
                  <textarea 
                    ...
                    className="w-full h-40 p-8 bg-slate-100/50 border-2 border-transparent focus:border-indigo-100/50 focus:bg-white rounded-[40px] text-base font-black italic tracking-tight transition-all resize-none placeholder:text-slate-400 placeholder:font-black focus:ring-0 shadow-inner" 
                  />
                  <div className="absolute bottom-8 right-8 flex items-center gap-6">
                     ...
                     <button className="w-16 h-16 bg-slate-950 rounded-2xl flex items-center justify-center text-white shadow-2xl hover:bg-indigo-600 transition-all hover:rotate-3 active:scale-90"><Send className="w-7 h-7 ml-1" /></button>
                  </div>
               </div>
               
               <div className="flex items-center justify-between px-4">
                  <p className="text-[11px] font-black text-slate-400 italic italic tracking-tight">Your care team monitors this broadcast 24/7 during active treatment.</p>
                  <p className="text-[10px] font-black text-slate-950 uppercase tracking-widest bg-slate-100 px-4 py-1.5 rounded-full">0 / 500 Safe Units</p>
               </div>
            </div>
         </div>
      </GlassCard>
