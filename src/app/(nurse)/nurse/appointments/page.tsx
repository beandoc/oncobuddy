    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-slate-100">
         <div className="space-y-1">
            <h1 className="text-5xl font-black font-outfit text-slate-900 tracking-tight italic italic">Care <span className="text-indigo-600 underline underline-offset-8 decoration-indigo-100">Schedule</span></h1>
            <p className="text-base font-bold text-slate-600 italic">Orchestrating {appointments.length} clinical sessions for current operating window.</p>
         </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 pt-4">
         ...
         <div className="space-y-8 lg:col-span-1">
            <div className="space-y-4">
               <p className="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] ml-2">Quick Stats</p>
               <div className="grid grid-cols-2 gap-4">
                  <GlassCard className="p-6 bg-white border-slate-100 text-center space-y-1 shadow-sm">
                     <p className="text-2xl font-black font-outfit text-indigo-600">4</p>
                     <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Today</p>
                  </GlassCard>
                  <GlassCard className="p-6 bg-white border-slate-100 text-center space-y-1 shadow-sm">
                     <p className="text-2xl font-black font-outfit text-emerald-600">12</p>
            ...
            <div className="space-y-4">
               <p className="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] ml-2">Session Type</p>
            ...
            <GlassCard className="bg-slate-950 text-white p-8 border-0 shadow-2xl rounded-[36px] relative overflow-hidden group">
               <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] relative z-10">Next Session in 12m</p>
               <h4 className="text-xl font-black font-outfit italic mt-2 relative z-10">Telehealth Queue</h4>
               <p className="text-xs text-slate-400 font-medium italic mt-2 relative z-10 leading-relaxed">
                  Michael Scott is already in the waiting room for the Toxicity Review.
               </p>
               <Button className="w-full mt-6 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest h-11 border-0 relative z-10 transition-all">
                  Join Room Now
               </Button>
               <Video className="absolute bottom-[-10px] right-[-10px] w-32 h-32 text-white/5 group-hover:scale-110 transition-transform duration-700" />
            </GlassCard>
         </div>

         {/* Middle & Right: Appointment Timeline */}
         <div className="lg:col-span-3 space-y-8">
            <h3 className="text-2xl font-black font-outfit text-slate-900 italic">Timeline <span className="text-indigo-600">Visibility</span></h3>
            ...
            <div className="space-y-6 relative ml-4 before:absolute before:left-0 before:top-4 before:bottom-4 before:w-px before:bg-slate-100">
               {appointments.map((apt, i) => (
                  <div key={apt.id} className="relative pl-12 group">
                     ...
                     <GlassCard className={`!p-0 border-slate-100 overflow-hidden hover:border-indigo-100 transition-all shadow-sm hover:shadow-xl ${apt.status === 'Ready to Join' ? "ring-2 ring-indigo-100" : ""}`}>
                        <div className="flex items-stretch">
                           <div className="p-8 flex items-center justify-center bg-slate-50/50 border-r border-slate-100 w-32 flex-shrink-0">
                              <p className="text-lg font-black font-outfit text-slate-900 italic text-center leading-none">{apt.time}</p>
                           </div>
                           <div className="flex-1 p-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                              <div className="space-y-1">
                                 ...
                                 <p className="text-xl font-black text-slate-900 italic group-hover:text-indigo-600 transition-colors uppercase">{apt.patient}</p>
                                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">{apt.id}</p>
                              </div>
                              <div className="space-y-1">
                                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Purpose/Blueprint</p>
                                 <p className="text-sm font-bold text-slate-700 italic leading-snug">{apt.purpose}</p>
                              </div>
                              <div className="flex items-center justify-end gap-3">
                                 <div className="text-right">
                                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${apt.status === 'Ready to Join' ? "bg-indigo-600 text-white shadow-lg" : "bg-slate-100 text-slate-500"}`}>
                                       {apt.status}
                                    </span>
                                 </div>
            ...
            <div className="p-10 border-2 border-dashed border-slate-100 rounded-[40px] text-center space-y-4">
               <div className="w-16 h-16 bg-slate-50 rounded-[28px] flex items-center justify-center mx-auto text-slate-200">
                  <CheckCircle2 className="w-8 h-8" />
               </div>
               <div className="space-y-1">
                  <p className="text-lg font-black text-slate-900 italic">End of Schedule</p>
                  <p className="text-xs text-slate-500 font-medium italic">All clinical sessions for the current operational window have been processed.</p>
               </div>
            </div>
         </div>
      </div>
