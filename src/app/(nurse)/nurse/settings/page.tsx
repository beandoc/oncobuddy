  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col gap-2">
         <h1 className="text-5xl font-black font-outfit text-slate-900 tracking-tight italic italic">Clinical <span className="text-indigo-600 underline underline-offset-4 decoration-indigo-200">Terminal</span></h1>
         <p className="text-base font-bold text-slate-600 italic">Configuration for {session.user.name} • Oncobuddy Clinical Navigator v2.4</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 pt-8">
         ...
         <div className="space-y-4">
            {sections.map((s) => (
                <button key={s.id} className={`w-full text-left p-6 rounded-[28px] border-2 transition-all group flex items-start gap-4 ${s.id === 'profile' ? "bg-white border-indigo-100 shadow-xl shadow-indigo-100/50" : "bg-slate-50/50 border-transparent hover:bg-white hover:border-slate-100"}`}>
                    ...
                    <div className="flex-1">
                        <p className={`text-sm font-black italic ${s.id === 'profile' ? "text-slate-900" : "text-slate-500 group-hover:text-slate-900"}`}>{s.title}</p>
         ...
         <div className="lg:col-span-2 space-y-10">
            ...
            {/* Profile Section */}
            <GlassCard className="!p-10 border-slate-100 space-y-10 rounded-[40px] shadow-sm">
                <div className="flex items-center justify-between border-b border-slate-50 pb-8">
                    <h3 className="text-2xl font-black font-outfit text-slate-900 italic">Identity <span className="text-slate-400">& Credentials</span></h3>
                    <Button variant="ghost" className="h-10 px-6 font-black text-[10px] uppercase tracking-widest text-indigo-600 hover:bg-indigo-50">Edit Profile</Button>
                </div>

                <div className="space-y-8">
                    <div className="flex items-center gap-8">
                        <div className="w-24 h-24 rounded-[32px] bg-slate-100 border-4 border-white shadow-xl relative overflow-hidden flex items-center justify-center">
                        ...
                        <div className="space-y-1">
                            <p className="text-xl font-black text-slate-900 italic">{session.user.name}</p>
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-none">Registered Nurse (RN) • ID-44920</p>
                            <label className="inline-flex items-center gap-2 mt-4 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                        <div className="space-y-2">
                            ...
                            <div className="h-12 px-5 bg-slate-50 border border-slate-100 rounded-2xl flex items-center text-sm font-bold text-slate-900 italic">
                                {session.user.email}
                            </div>
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Clinical Extension</label>
                            <div className="h-12 px-5 bg-slate-50 border border-slate-100 rounded-2xl flex items-center text-sm font-bold text-slate-900 italic italic">
                                +1 (555) 092-1182
                            </div>
                        </div>
                    </div>
            ...
            <GlassCard className="!p-10 border-slate-100 space-y-10 rounded-[40px] shadow-sm">
                <div className="flex items-center justify-between border-b border-slate-50 pb-8">
                    <h3 className="text-2xl font-black font-outfit text-slate-900 italic">Alert <span className="text-indigo-600">Escalation</span></h3>
                    <div className="bg-amber-50 text-amber-700 px-4 py-1.5 rounded-2xl flex items-center gap-2">
                        <AlertCircle className="w-3.5 h-3.5" />
                        <span className="text-[10px] font-black uppercase tracking-widest">Urgent Only</span>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    {[
                        ...
                        <div key={i} className="flex items-center justify-between p-6 bg-slate-50/50 rounded-3xl border border-slate-100 hover:bg-white transition-all group">
                            <div className="space-y-1">
                                <p className="text-sm font-black text-slate-900 italic group-hover:text-indigo-600 transition-colors">{pref.label}</p>
